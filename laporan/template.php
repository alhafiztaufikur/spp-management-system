...
if($template==='setoran'){
    ...
    $summaryCards=[];
    $resultRangeLabel=number_format((int)($report['settlement']['payment_count']??0)).' transaksi pembayaran';
}
...
<div class="recap-report-header"><div class="recap-report-copy"><span class="recap-class-overline">Laporan Global</span><h1><?= report_e($report['title']) ?></h1><p><?= report_e($report['subtitle']) ?> · <?= number_format($totalRows) ?> <?= report_e($paginationUnit) ?> hasil filter.</p></div>
...
<?php if($template==='setoran'): ?><section class="report-settlement-component-card"><div class="report-settlement-section-head"><h3>Komponen Pembayaran</h3><div class="report-settlement-component-total"><span>Total Bersih</span><strong><?= report_money($report['component_total']??0) ?></strong></div></div><div class="table-container report-settlement-component-table"><table class="payment-table"><thead><tr><th>No</th><th>Komponen</th><th>Nominal</th></tr></thead><tbody><?php foreach($componentRows as $index=>$component): ?><tr><td><?= $index+1 ?></td><td><?= report_e($component['komponen']) ?></td><td class="<?= (float)$component['nominal']<0?'report-negative':'' ?>"><?= report_money($component['nominal']) ?></td></tr><?php endforeach; ?></tbody><tfoot><tr><th colspan="2">Total Bersih</th><th class="<?= (float)($report['component_total']??0)<0?'report-negative':'' ?>"><?= report_money($report['component_total']??0) ?></th></tr></tfoot></table></div></section><section class="report-settlement-method-summary"><h3>Metode Pembayaran</h3><div class="report-settlement-method-grid"><?php foreach($methodSummary as $method): ?><div class="report-settlement-method-card"><span><?= report_e($method['metode']) ?></span><strong><?= report_money($method['nominal']) ?></strong></div><?php endforeach; ?></div></section><section class="report-settlement-total-card"><div><span>TOTAL SETORAN</span></div><strong><?= report_money($report['total_setoran']??$report['component_total']??0) ?></strong></section><?php endif; ?>
<?php if($template!=='setoran'): ?>
<?php if($billingGroupedView): ?>
<div class="table-container report-billing-group-wrap">
  <table class="payment-table report-billing-group-table">
    <thead><tr><th>No</th><th>Siswa</th><th>Kelas</th><th>Ringkasan</th><th>Rincian Tagihan</th></tr></thead>
    <tbody>
    <?php if(!$pagination['rows']): ?>
      <tr><td colspan="5"><div class="empty-state"><p>Tidak ada data</p><span>Ubah filter atau lengkapi data tagihan yang dibutuhkan.</span></div></td></tr>
    <?php else: foreach($pagination['rows'] as $index=>$student): ?>
      <tr class="report-billing-student-row">
        <td class="report-billing-number" data-label="No"><?= (($pagination['page']-1)*$pagination['per_page'])+$index+1 ?></td>
        <td class="report-billing-student" data-label="Siswa">
          <strong><?= report_e($student['nama']) ?></strong>
          <span class="badge-nis"><?= report_e($student['nis']) ?></span>
          <?php if($student['nis_diknas']!==''): ?><small class="report-secondary-id">Diknas <?= report_e($student['nis_diknas']) ?></small><?php endif; ?>
          <small><?= number_format((int)$student['item_count']) ?> rincian tagihan</small>
        </td>
        <td class="report-billing-class" data-label="Kelas"><span class="kelas-badge"><?= report_e($student['kelas']) ?></span></td>
        <td class="report-billing-summary" data-label="Ringkasan">
          <div class="report-billing-summary-item is-primary"><span>Total Tagihan</span><strong><?= report_money($student['total_tagihan']) ?></strong></div>
          <div class="report-billing-summary-item"><span>Terbayar</span><strong><?= report_money($student['total_terbayar']) ?></strong></div>
          <div class="report-billing-summary-item"><span>Sisa</span><strong><?= report_money($student['total_sisa']) ?></strong></div>
        </td>
        <td class="report-billing-details" data-label="Rincian Tagihan">
          <div class="report-billing-item-list">
          <?php foreach($student['items'] as $item): ?>
            <article class="report-billing-item">
              <div class="report-billing-item-copy"><strong><?= report_e($item['komponen']) ?></strong><span><?= report_e($item['periode']) ?><?php if($item['periode']!==$item['tahun_ajaran']): ?> · TA <?= report_e($item['tahun_ajaran']) ?><?php endif; ?></span></div>
              <div class="report-billing-item-values">
                <span><small>Tagihan</small><strong><?= report_money($item['tagihan']) ?></strong></span>
                <span><small>Terbayar</small><strong><?= report_money($item['terbayar']) ?></strong></span>
                <span><small>Sisa</small><strong><?= report_money($item['sisa']) ?></strong></span>
              </div>
              <span class="report-status status-<?= report_e(report_status_key((string)$item['status'])) ?>"><?= report_e($item['status']) ?></span>
            </article>
          <?php endforeach; ?>
          </div>
        </td>
...
</main></div><script src="../assets/js/app.js?v=7.7"></script><?php if($template==='per-item'): ?><script>
document.addEventListener('DOMContentLoaded',function(){
