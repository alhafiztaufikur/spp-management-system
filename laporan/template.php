...
if($isCashRecap){
    ...
    $cashSummaryItems=$isSppDepositRecap
        ? [
            ['label'=>'Saldo Awal','value'=>(float)($report['deposit_summary']['saldo_awal']??0),'type'=>'money'],
            ['label'=>'Penerimaan','value'=>(float)($report['deposit_summary']['penerimaan']??0),'type'=>'money'],
            ['label'=>'Dipakai SPP','value'=>(float)($report['deposit_summary']['penggunaan']??0),'type'=>'money'],
            ['label'=>'Pengembalian','value'=>(float)($report['deposit_summary']['pengembalian']??0),'type'=>'money'],
        ]
        : ($isSavingsCashRecap
    ...
    $cashTotalLabel=$isSppDepositRecap?'SALDO AKHIR':($isSavingsCashRecap?'MUTASI BERSIH':'TOTAL SETORAN');
    $cashTotalValue=$isSppDepositRecap?(float)($report['deposit_summary']['saldo_akhir']??0):($isSavingsCashRecap?(float)($report['mutasi_bersih']??0):(float)($report['total_setoran']??$report['component_total']??0));
    $summaryCards=[];
    $resultRangeLabel=$isSppDepositRecap
        ? number_format($totalRows).' mutasi titipan'
        : ($isSavingsCashRecap
...
<?php if(!$isCashRecap): ?><div class="field-row"><label class="field-label">Kelas/Rombel</label><select class="field-input field-select" name="kelas"><option value="" <?= $filters['kelas']===''?'selected':'' ?>>Semua Kelas</option><?php foreach($classLevels as $level): ?><option value="tingkat:<?= $level ?>" <?= $filters['kelas']==='tingkat:'.$level?'selected':'' ?>>Semua Kelas <?= $level ?></option><?php endforeach; ?><?php foreach($classes as $class): ?><option value="rombel:<?= (int)$class['id'] ?>" <?= $filters['kelas']==='rombel:'.((int)$class['id'])?'selected':'' ?>><?= report_e(class_label($class)) ?></option><?php endforeach; ?></select></div><?php endif; ?>
<?php if(in_array($template,['penerimaan','tabungan-siswa','setoran','kas-tabungan'],true)): ?><div class="field-row"><label class="field-label">Operator</label><select class="field-input field-select" name="operator"><option value=""><?= report_e($operatorFilterLabel) ?></option><?php foreach($operatorOptions as $operator): ?><option value="<?= (int)$operator['id'] ?>" <?= $filters['operator']==(string)$operator['id']?'selected':'' ?>><?= report_e($operator['nama']) ?></option><?php endforeach; ?></select></div><?php endif; ?>
...
<?php if(!$isCashRecap): ?><div class="field-row full-span report-student-field"><label class="field-label" for="global-siswa-search">Cari Siswa (Nama / NIS / NIS Diknas)</label><div class="search-box"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input type="text" id="global-siswa-search" data-student-search data-student-list="global-siswa-list" data-student-select-callback="selectReportStudentSearchOption" data-student-query-target="global-student-query" data-student-class-filter="[name=kelas]" value="<?= report_e($studentSearchDisplay) ?>" placeholder="Ketik nama, NIS, atau NIS Diknas..." autocomplete="off"><input type="hidden" id="global-student-query" name="q" value="<?= report_e($filters['q']) ?>"></div><datalist id="global-siswa-list"><?php foreach($studentOptions as $studentOption): $studentLevel=(string)($studentOption['master_tingkat']?:$studentOption['KELAS']); $studentClassLabel=class_label(['tingkat'=>$studentLevel,'kode_rombel'=>$studentOption['kode_rombel']??'BELUM','is_placeholder'=>$studentOption['is_placeholder']??1]); ?><option value="<?= report_e($studentOption['NAMA']) ?>" data-nis="<?= report_e($studentOption['NO_INDUK']) ?>" data-diknas="<?= report_e((string)($studentOption['NO_induk_diknas']??'')) ?>" data-nama="<?= report_e($studentOption['NAMA']) ?>" data-kelas="<?= report_e($studentClassLabel) ?>" data-kelas-id="<?= (int)($studentOption['master_kelas_id']??0) ?>" data-tingkat="<?= report_e($studentLevel) ?>"><?= report_e($studentOption['NAMA']) ?> (<?= report_e($studentClassLabel) ?>)</option><?php endforeach; ?></datalist></div><div class="field-row report-per-page-field"><label class="field-label"><?= $billingGroupedView?'Siswa/Halaman':'Baris/Halaman' ?></label><select class="field-input field-select" name="per_page"><?php foreach([25,50,100] as $size): ?><option <?= $filters['per_page']===$size?'selected':'' ?>><?= $size ?></option><?php endforeach; ?></select></div><?php endif; ?><div class="report-filter-actions"><button class="btn btn-primary" type="submit">Tampilkan Rekap</button><a class="btn btn-ghost" href="template.php?template=<?= urlencode($template) ?>">Reset</a></div></form></div>
<?php if(isset($report['error'])): ?><div class="alert alert-error report-template-alert"><?= report_e($report['error']) ?></div><?php endif; ?>
<div class="recap-period-strip report-template-strip"><div class="report-strip-copy"><strong><?= report_e($registry[$template]['label']) ?></strong><span><?= report_e($report['subtitle']) ?></span></div><div class="report-strip-tools"><span class="report-result-pill"><?= report_e($resultRangeLabel) ?></span><div class="report-export-actions" aria-label="Export laporan"><a class="btn btn-ghost report-export-btn" target="_blank" rel="noopener" href="export_global.php?<?= http_build_query(array_merge($exportQuery,['format'=>'print'])) ?>">Cetak</a><a class="btn btn-ghost report-export-btn" target="_blank" rel="noopener" href="export_global.php?<?= http_build_query(array_merge($exportQuery,['format'=>'preview'])) ?>">PDF</a><a class="btn btn-primary report-export-btn" target="_blank" rel="noopener" href="export_global.php?<?= http_build_query(array_merge($exportQuery,['format'=>'excel'])) ?>">Excel</a></div></div></div>
...
</main></div><script src="../assets/js/app.js?v=10.4"></script><?php if($template==='per-item'): ?><script>
document.addEventListener('DOMContentLoaded',function(){
