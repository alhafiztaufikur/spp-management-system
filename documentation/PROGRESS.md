...
Koreksi status 27 Agustus: regression disposable terbaru adalah `REG-FINAL-008` pada clone `db_spp_audit_20260827_035115_suite_5476` (18/18 PASS), termasuk probe time-based SQLi read-only pada field filter yang tepat; referensi `REG-FINAL-003` sampai `007` pada baris historis di atas dipertahankan untuk jejak audit.

Regression evidence terbaru yang mencakup seluruh hardening dan oracle (termasuk pagination scalar boundary, protected rekap filter boundary, formula-prefix/control-character, time-based SQLi read-only, lifecycle, audit, payment/savings, security, sanitasi fallback error, pemeriksaan CSRF sebelum pembuatan tahun ajaran, atomisitas ensure tahun, dan validasi scalar alasan audit) adalah `regression-20260827_035115-time-sqli-final` (`REG-FINAL-008`, 18/18 PASS; clone `db_spp_audit_20260827_035115_suite_5476`). Run ini menguji field `nis`, `search`, dan `q` dengan pengukuran latency baseline versus payload `SLEEP(3)`; ringkasan tersanitasi dipertahankan di luar repository dan bukti mentah telah dipurge.
...
Retest pasca-commit `a91edab` menambahkan `REG-FINAL-005`, `DBM-004`, `DEP-REHEARSAL-009`, dan `DBPOST-004`; commit `b983c67` memperbaiki harness concurrency dan menambahkan `CONC-004` (25/25 PASS). Retest terbaru `REG-FINAL-008` kembali lulus 18/18 setelah koreksi field probe time-based SQLi; `REG-FINAL-006` dan `REG-FINAL-007` dipertahankan sebagai histori sinkronisasi/probe. Seluruhnya PASS pada environment disposable/snapshot dengan database utama `db_spp` tetap tidak disentuh.
